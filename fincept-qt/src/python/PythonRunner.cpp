...
void PythonRunner::run(const QString& script, const QStringList& args, Callback cb, StreamCallback on_line, int timeout_ms, bool is_background) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(
            this,
            [this, script, args, cb_copy = std::move(cb_copy), on_line_copy = std::move(on_line_copy), timeout_ms, is_background]() mutable {
                run(script, args, std::move(cb_copy), std::move(on_line_copy), timeout_ms, is_background);
            },
            Qt::QueuedConnection);
        return;
    }
    ...
    if (is_background) {
        if (timeout_ms == 60000) timeout_ms = 0; // Default to infinite timeout for background
        bg_queue_.enqueue({script, args, std::move(cb), std::move(on_line), timeout_ms, true});
    } else {
        queue_.enqueue({script, args, std::move(cb), std::move(on_line), timeout_ms, false});
    }
    start_next();
}
...
void PythonRunner::run_code(const QString& code, Callback cb, int timeout_ms) {
    // Same thread-affinity guard as run(). See the comment there.
    if (QThread::currentThread() != this->thread()) {
        Callback cb_copy = std::move(cb);
        QMetaObject::invokeMethod(
            this,
            [this, code, cb_copy = std::move(cb_copy), timeout_ms]() mutable {
                run_code(code, std::move(cb_copy), timeout_ms);
            },
            Qt::QueuedConnection);
        return;
    }
    ...
    queue_.enqueue({"__code__:" + temp_path, {}, std::move(cb), {}, timeout_ms});
    start_next();
}
...
void PythonRunner::start_next() {
    ...
    while ((active_count_ < max_concurrent_ && !queue_.isEmpty()) ||
           (bg_active_count_ < max_bg_concurrent_ && !bg_queue_.isEmpty())) {

        QueuedRequest req;
        bool is_bg = false;

        if (active_count_ < max_concurrent_ && !queue_.isEmpty()) {
            req = queue_.dequeue();
        } else if (bg_active_count_ < max_bg_concurrent_ && !bg_queue_.isEmpty()) {
            req = bg_queue_.dequeue();
            is_bg = true;
        } else {
            break;
        }
        ...
        connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
                [this, proc, cb = std::move(cb), script_name, is_code, temp_file, is_bg](int exit_code, QProcess::ExitStatus) {
        ...
        connect(proc, &QProcess::errorOccurred, this, [this, proc, cb, is_code, temp_file, is_bg](QProcess::ProcessError) {
        ...
        LOG_INFO("Python", QString("Running [%6] (%1/%2 active, %3 queued): %4 %5")
                               .arg(is_bg ? bg_active_count_ : active_count_)
                               .arg(is_bg ? max_bg_concurrent_ : max_concurrent_)
                               .arg(is_bg ? bg_queue_.size() : queue_.size())
                               .arg(python_exe)
                               .arg(script_path)
                               .arg(is_bg ? "BG" : "UI"));
        ...
        if (req.timeout_ms > 0) {
            ...
            connect(timer, &QTimer::timeout, proc, [proc, script_path]() {
                if (proc->state() == QProcess::Running) {
                    LOG_WARN("Python", "Script timed out (" + script_path + "), killing process.");
                    proc->kill();
                }
            });
