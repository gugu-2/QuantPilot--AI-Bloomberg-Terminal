...
class PythonRunner : public QObject {
  ...
  public:
    ...
    void run(const QString& script, const QStringList& args, Callback cb,
             StreamCallback on_line = {}, int timeout_ms = 60000, bool is_background = false);

    /// Run arbitrary Python code (for notebook/colab cells).
    /// Creates a temp file, executes it, returns stdout/stderr.
    void run_code(const QString& code, Callback cb, int timeout_ms = 60000);
  ...
  private:
    ...
    QQueue<QueuedRequest> bg_queue_;
    ...
    static constexpr int DEFAULT_MAX_BG_CONCURRENT = 2;
    int max_bg_concurrent_ = DEFAULT_MAX_BG_CONCURRENT;
    int bg_active_count_ = 0;
