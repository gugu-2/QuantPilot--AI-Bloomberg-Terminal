...
int main(int argc, char* argv[]) {
    ...
    bool is_daemon = false;
    {
        for (int i = 1; i < argc; ++i) {
            if (qstrcmp(argv[i], "--profile") == 0 && i < argc - 1) {
                fincept::ProfileManager::instance().set_active(QString::fromUtf8(argv[i + 1]));
            } else if (qstrcmp(argv[i], "--daemon") == 0 || qstrcmp(argv[i], "--headless") == 0) {
                is_daemon = true;
            }
        }
    ...
    app.setApplicationName("QuantPilot");
    app.setOrganizationName("QuantPilot");
    app.setOrganizationDomain("quantpilot.app");
...
#    define FINCEPT_VERSION_STRING "0.0.0-dev"
    ...
    const QString profile_key = QString("QuantPilot-%1").arg(fincept::ProfileManager::instance().active());
    ...
    // Initialize the Dynamic Plugin System
    fincept::core::plugins::PluginManager::instance().init();
    ...
    QTimer::singleShot(0, qApp, []() {
        ...
        // Alt Data
        (void)fincept::services::alt_data::AltDataService::instance();
    ...
    LOG_INFO("App", "QuantPilot v1.0.0 starting...");
    ...
    // ── Headless Daemon Check ────────────────────────────────────────────────
    if (is_daemon) {
        LOG_INFO("App", "Running in Headless Daemon mode. Bypassing UI instantiation.");
        static fincept::services::daemon::DaemonServer daemon_server;
        daemon_server.start(8080);
        return app.exec();
    }
