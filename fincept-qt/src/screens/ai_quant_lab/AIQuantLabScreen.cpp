...
QWidget* AIQuantLabScreen::build_right_sidebar() {
    ...
    // Trading Agents launcher card
    auto* ta_card = new QWidget(right_panel_);
    ta_card->setObjectName("aqTaCard");
    auto* tvl = new QVBoxLayout(ta_card);
    tvl->setContentsMargins(10, 10, 10, 10);
    tvl->setSpacing(8);

    auto* ta_title = new QLabel(tr("TRADING AGENTS"), ta_card);
    ta_title->setObjectName("aqStatsTitle"); // reuse same style
    tvl->addWidget(ta_title);

    auto* ta_ticker = new QLineEdit(ta_card);
    ta_ticker->setPlaceholderText("Ticker (e.g. NVDA)");
    ta_ticker->setStyleSheet("padding: 4px; border: 1px solid #555; border-radius: 2px;");
    tvl->addWidget(ta_ticker);

    auto* ta_btn = new QPushButton(tr("Launch Agent"), ta_card);
    ta_btn->setStyleSheet("background: #007ACC; color: white; padding: 6px; font-weight: bold; border-radius: 2px;");
    tvl->addWidget(ta_btn);
    
    vl->addWidget(ta_card);

    connect(ta_btn, &QPushButton::clicked, this, [this, ta_ticker]() {
        QString ticker = ta_ticker->text().trimmed();
        if (ticker.isEmpty()) return;
        ta_ticker->clear();
        
        LOG_INFO("AIQuantLab", "Launching TradingAgent for " + ticker);
        PythonRunner::instance().run("tradingagents_runner.py", {ticker}, [ticker](const PythonResult& res) {
            if (res.success) {
                QJsonParseError err;
                QJsonDocument doc = QJsonDocument::fromJson(res.output.toUtf8(), &err);
                if (err.error == QJsonParseError::NoError && doc.isObject()) {
                    core::events::EventBus::instance().publish("ai.trade.proposed", doc.object());
                    LOG_INFO("AIQuantLab", "Trade proposal dispatched for " + ticker);
                } else {
                    LOG_ERROR("AIQuantLab", "Agent parse error: " + res.output);
                }
            } else {
                LOG_ERROR("AIQuantLab", "Agent failed: " + res.error);
            }
        }, {}, 0, true);
    });
...
void AIQuantLabScreen::refresh_theme() {
    ...
    if (auto* ta_card = right_panel_->findChild<QWidget*>("aqTaCard"))
        ta_card->setStyleSheet(
            QString("background:%1; border:1px solid %2; border-radius:3px;").arg(BG_RAISED(), BORDER_DIM()));
