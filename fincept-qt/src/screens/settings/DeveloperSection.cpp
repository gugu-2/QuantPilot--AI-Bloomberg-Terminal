...
#include "screens/devtools/ResourceManager.h"
...
DeveloperSection::DeveloperSection(QWidget* parent) : QWidget(parent) {
    ...
    sep2->setStyleSheet(QString("color:%1;").arg(ui::colors::BORDER_DIM()));
    sep2->setFixedHeight(1);
    vl->addWidget(sep2);

    auto* rm_title = new QLabel(tr("Resource Manager (System Profiler)"));
    rm_title->setStyleSheet(section_title_ss());
    vl->addWidget(rm_title);

    vl->addWidget(new devtools::ResourceManager(this), 0);
}
